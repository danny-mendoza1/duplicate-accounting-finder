...
import { sortRecordsWithStaticFirst } from '../helpers/sorting';
...
export function ResultsTable({ groups, vendorScope = null, showHeader = true }: ResultsTableProps) {
  const { toggleSort, getSortConfig } = useTableSort();
  ...
  return (
    <section>
      ...
      {groups.map((group, idx) => {
        const sortConfig = getSortConfig(group.key);
        const sortedItems = sortRecordsWithStaticFirst(group.items, sortConfig);
              ...
              <thead>
                <tr>
                  ...
                  <SortableHeader
                    ...
                    onSort={() => toggleSort(group.key, 'type')}
                  >
                    Type
                  </SortableHeader>
                  <SortableHeader
                    ...
                    currentColumn={sortConfig?.column}
                    currentDirection={sortConfig?.direction}
                    onSort={() => toggleSort(group.key, 'date')}
                  >
                    Date
                  </SortableHeader>
                  <th>Property</th>
                  <th className="align-right">Amount</th>
                  <th className="align-center">Color</th>
                  <th>Memo</th>
                  <th>Vendor</th>
                </tr>
              </thead>
                ...
                {sortedItems.map((record, i) => {
                  ...
                  return (
                    <tr key={i} className={rowClass}>
                      <td>{sourceLabel}</td>
                      <td>
                        <span style={{ color: typeColor }} className="type-badge">
                          {typeValue}
                        </span>
                      </td>
                      <td>{dateVal}</td>
                      ...
                      <td className="align-right">
                        {record.amountCents == null
                          ? ''
                          : formatUSDFromCents(Math.abs(record.amountCents))}
                      </td>
                      ...
                      <td className="memo-column">{memoVal}</td>
