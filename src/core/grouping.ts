import type { CsvRecord, UiGroup, VendorGroup, DuplicateDetectionResult } from '../types';
...
export function buildGroupsCsvToCsv(
  ...
  for (const bill of validBills) {
    ...
    if (!groupsMap.has(k)) {
      ...
      groupsMap.get(k)!.billsRows.push(bill);
    }
  }
...
// Organize duplicate groups by vendor
export function groupByVendor(
  ...
  for (const group of duplicateGroups) {
    ...
    const firstRecord = group.billsRows[0] || group.buildiumRows[0];
    ...
    vendorGroup.billsCount += group.billsRows.length;
    vendorGroup.buildiumCount += group.buildiumRows.length;
  }
