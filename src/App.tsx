...
export default function App() {
  ...
  const [duplicateGroups, setDuplicateGroups] = useState<Array<{ key: string; items: AnyRecord[] }>>([]);
  ...
  const [vendorScope, setVendorScope] = useState<{ vendorRaw: string; vendorNorm: string } | null>(null);
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      ...
      setCsvColumnKeys(allCsvRecords[0] ? Object.keys(allCsvRecords[0].raw as Record<string, unknown>) : []);
      setJsonColumnKeys(allJsonRecords[0] ? Object.keys(allJsonRecords[0].raw as Record<string, unknown>) : []);
      setFirstRowPreview(
      ...
      setDroppedRowCount(
        scopedCsvRecords.length + allJsonRecords.length - (usableCsvRecords.length + usableJsonRecords.length)
      );
      ...
      if (e instanceof Error) {
        ...
        let suggestion = "Check that your data format is correct and all required columns are present.";
        ...
        if (e.message.includes("JSON")) {
          ...
          suggestion = "Check that your file contains columns named: Property, Amount, and Vendor (case-sensitive).";
        }
